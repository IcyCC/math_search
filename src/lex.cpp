...
string normal[NUM_NORMAL]={"-", "+", "<", ">", "=", "\\neq", "\\leq", "\\le", "\\geq", "\\ge", "\\times", "^",
                           "\\approx", "\\div", "\\frac", "\\cdot", "\\pi", "\\vert", "|", "{", "}", "(", ")", "\\mid","\\circ"};
...
int HandleFrac(vector<Token> &vec, string &s, string number);
...
void HandleString(string s, vector<Token> &vec){
    ...
    while(temp.size()!=0){
        ...
        if(mn<NUM_NORMAL && normal[mn]=="\\frac"){
            ...
            //\vert=>|
        else if(mn<NUM_NORMAL && (normal[mn]=="\\vert" || normal[mn]=="|" || normal[mn]=="\\mid")){
            temp.erase(0, normal[mn].size() );
            //			cout << temp<<endl;
            HandleVert(vec, temp);
            continue;
...
void HandleVert(vector<Token> &vec, string &temp){
    ...
    while(temp.size()!=0 && StartWith(temp,"|")!=1 && StartWith(temp,"\\vert")!=1 && StartWith(temp,"\\mid")!=1){
        value+=temp.at(0);
        temp.erase(0,1);
    }
    ...
    else if(StartWith(temp,"\\mid")){
        temp.erase(0,sizeof("\\mid")-1);
        value+="|";
    }
    else if(StartWith(temp,"\\vert")){
